...
class MainPane(QFrame):
     def __init__(self, path):
          ...
          self.linkinput = LinkInputPane(self.mainPaneLayout, path=self.path)
          
          self.mainPaneLayout.addWidget(self.linkinput)

class LinkInputPane(QFrame):
     def __init__(self, layout: QVBoxLayout, path):
          super().__init__()
          self.layout_ = layout
          self.path = path
          self.setContentsMargins(0,0,0,0)
          ...
          self.input.setObjectName("pathInputDirectory")
     ...
     def startDownload(self):
          if self.input.text() != "":
               self.downloadProgress = DownloadProgressBarContainer(self.input, path=self.path)
...
class DownloadProgressBarContainer(QFrame):
     def __init__(self, input: QLineEdit, path) -> None:
          super().__init__()
          ...
          self.cancel_download_button.setIcon(QIcon("./icons/cancel.svg"))
     ...
     def Download(self):
          ...
          save_loc = f'{self.path}/{unquote(file_name)}'
     ...
     def contextMenu(self, pos: QPoint):
          ...
          styleSheet = open("./venus/venus_st.css")
